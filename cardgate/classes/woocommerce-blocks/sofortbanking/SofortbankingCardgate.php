 ...
 */
final class SofortbankingCardgate extends AbstractPaymentMethodType {
	 ...
	 * @var string
	 */
	protected $name = 'cardgatesofortbanking';
	...
	public function get_payment_method_script_handles() {
		...
		wp_register_script(
			'wc_payment_method_cardgatesofortbanking',
			$url . 'build/index.js',
			array(),
			'1020349.' . wp_rand( 0, 9999 ),
			true
		);
		return array( 'wc_payment_method_cardgatesofortbanking' );
	}
	...
	public function get_payment_method_data() {
		return array(
			...
			'icon'         => $this->iconpath . 'sofortbanking.svg',
			'show_icon'    => $this->settings['show_icon'],
			'supports'     => array( 'products' ),
		);
	}
	private function get_settings() {
		$settings              = get_option( 'woocommerce_cardgatesofortbanking_settings', array() );
