 ...
 */
final class SpraypayCardgate extends AbstractPaymentMethodType {
	 ...
	 * @var string
	 */
	protected $name = 'cardgatespraypay';
	...
	public function get_payment_method_script_handles() {
		...
		wp_register_script(
			'wc_payment_method_cardgatespraypay',
			$url . 'build/index.js',
			array(),
			'1020349.' . wp_rand( 0, 9999 ),
			true
		);
		return array( 'wc_payment_method_cardgatespraypay' );
	}
	...
	public function get_payment_method_data() {
		return array(
			...
			'icon'         => $this->iconpath . 'spraypay.svg',
			'show_icon'    => $this->settings['show_icon'],
			'supports'     => array( 'products' ),
		);
	}
	private function get_settings() {
		$settings              = get_option( 'woocommerce_cardgatespraypay_settings', array() );
